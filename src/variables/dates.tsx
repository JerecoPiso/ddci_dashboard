export const getProdDate = (date: Date | undefined) => {
    const _current_date = new Date();
     const _prod_date = date === undefined ? `${_current_date.getFullYear()}-${_current_date.getMonth() < 9 ? '0'+Number(_current_date.getMonth()+1) : Number(_current_date.getMonth()+1)}-${_current_date.getDate() < 10 ? '0'+_current_date.getDate() : _current_date.getDate()}` : 
